...
groups = [
    ...
    Group(
        "2:www",
        matches=[
            Match(wm_class=["Firefox", "Google-chrome", "Vivaldi-snapshot", "Chromium"])
        ],
    ),
    Group(
        "3:dev",
    ...
    Group("5:none"),
    Group("6:none"),
    Group("7:IM", matches=[Match(wm_class=["Slack"])]),
    Group("8:mail", matches=[Match(wm_class=["Thunderbird", "openwmail"])]),
    Group("9:SNS", matches=[Match(wm_class=["mikutter", "Franz", "Station"])]),
    Group("0:stat"),
]
...
bars = {
    "screen1": {"top": None, "bottom": None, "screen_name": "S1"},
    "screen2": {"top": None, "bottom": None, "screen_name": "S2"},
}
